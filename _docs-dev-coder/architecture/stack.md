...
- `next.config.ts`: **sem** `ignoreBuildErrors`. Domínio `comprasparaguay.online` é greenfield (primeiro
  deploy) — **sem** `redirects()` de nível de config. Existem porém redirects em nível de **página**
  (ex.: `app/o-que-fazer-em-foz/[slug]/page.tsx` usa `permanentRedirect` para `/atrativos/[slug]` —
  rota legada de catálogo antigo). `headers()` aplica `X-Robots-Tag: noindex, nofollow` em toda a
